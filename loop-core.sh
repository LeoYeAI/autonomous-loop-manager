...
case "$1" in
  ...
  echo "  recall manifest <user_id> [--budget N] - 注入用的一行式清单"
  echo "  recall expand <type> <id>              - 按需展开单条全文"
  echo "  recall search <keyword> [limit]        - 跨记忆关键词搜索"
    ;;
  *)
    echo "Usage: loop <command> [args]"
