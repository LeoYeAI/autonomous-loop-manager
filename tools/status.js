...
function showStatus(userId) {
  ...
  const goals = readJson('goals.json').filter((g) => g.user_id === userId);
  const strategies = readJson('strategies.json').filter((s) => s.user_id === userId);
  ...
  if (active.length > 0) {
    ...
    active.forEach((g) => {
      ...
      if (g.current_step) console.log(`    当前: ${g.current_step}`);
    });
  }
  ...
  if (failed.length > 0) {
    ...
    failed.slice(-3).forEach((r) => {
      console.log(`  • ${r.task_summary}: ${r.reflection.slice(0, 50)}...`);
    });
  }
